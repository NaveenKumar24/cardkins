...
export class UserseVisitingCardsMetaComponent implements  OnInit{
  ...
  constructor(private http: HttpClient,
   ...
   ngOnInit() {
      ...
      this.generateToken();   
      this.Encrypt();
      const encrypted = this.TokenService.EncryptedData(JSON.stringify(this.UserCardDetails));
  ...
  Encrypt() {
      ...
      var encrypted = CryptoJS.AES.encrypt(JSON.stringify(this.UserCardDetails), key128Bits1000Iterations, 
        { 
  ...
  postData(encryptedData) {
    ...
    console.log(this.url + 'WebAdminPanel/UserCardDetailsReport');
//UserEVistingCardMetaTagReport
    let body ={"request": encryptedData}
    this.http.post(this.url + 'WebAdminPanel/UserCardDetailsReport',body,{ headers: headers})
    .subscribe(
    res =>{
        this.res = res;
  ...
  DecryptedData(response) {
     ...
     var decrypted = CryptoJS.AES.decrypt(response, key128Bits1000Iterations, { mode: CryptoJS.mode.CBC, iv: iv, padding: CryptoJS.pad.Pkcs7 });
      ...
      let TableData = DecryptOutput.responseValue.UserEcardStatusReportData;
