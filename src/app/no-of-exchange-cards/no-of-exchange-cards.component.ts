...
@Component({
  selector: 'app-no-of-exchange-cards',
  templateUrl: './no-of-exchange-cards.component.html',
  styleUrls: ['./no-of-exchange-cards.component.scss']
})
export class NoOfExchangeCardsComponent implements OnInit {
  status: StausType[] = [
  ...
  ngOnInit(): void {
    ...
    if (this.noOfCards == undefined && result == undefined) {
      this.router.navigate(['../userListReport/countOfExchangeCards']);
    }

    if (this.noOfCards == undefined && result == 'countOfExchangeCards') {
      this.router.navigate(['../userListReport/countOfExchangeCards']);
    }    
